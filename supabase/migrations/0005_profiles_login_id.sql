...
-- directly in an UPDATE statement (SQLSTATE 42P20), and the target table
-- can't be referenced inside a FROM join (only in WHERE) — SQLSTATE 42P01.
with numbered as (
...
set login_id =
  ...
  || lpad(n.rn::text, 4, '0')
from public.companies c, numbered n
